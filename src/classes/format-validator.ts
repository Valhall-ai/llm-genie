class FormatValidator {
    ...
    numberedList(input: string, defaults: any = {}): any {
    ...
    bulletedList(input: string, defaults: any): any {
    ...
    itemInArray(input: string, items: string[]): any {
    ...
    constrainedArray(testedArray: string[], optionsArray: string[], defaults: any = { debug: false, singleChoice: false }): any {
        console.debug('constrainedArray\n', testedArray, optionsArray, defaults)
        let validList = true;

        if (testedArray && optionsArray) {
            if (testedArray.length === 0) {
                if (defaults.debug) console.log('Invalid list: Empty.');
                return false;
            }
            if (defaults.singleChoice && testedArray.length > 1) {
                if (defaults.debug) console.log('Invalid list: Only one option can be selected.');
                return false;
            }
